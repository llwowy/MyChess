...
Rook::Rook(const std::string& _id, std::string _color, std::vector<BoardTile*>& board) : Piece(_id, _color) {
	set_Piece(board, _id);
	...
	if (_color == "B") { setTextureRect(sf::IntRect(16, 16, 16, 16)); }	//zmiany kolorów zale¿nie od Pawn_color
	if (_color == "G") { setTextureRect(sf::IntRect(95, 16, 16, 16)); }
	if (_color == "Wh") { setTextureRect(sf::IntRect(16, 112, 16, 16)); }
	if (_color == "Bl") { setTextureRect(sf::IntRect(96, 11, 16, 16)); }
	setScale(7, 7);
}

void Rook::move(std::vector<BoardTile*>& board, const sf::Vector2i& mouse_position) {
	if (get_is_selected()) {
