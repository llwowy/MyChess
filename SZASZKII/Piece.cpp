...
bool Piece::get_tied() { return tied; }

void Piece::set_tied(bool b) {
	tied = b;
}

bool Piece::get_delectable() {
	return deletable;
}

void Piece::set_delectable(bool b) {
	deletable = b;
}

bool Piece::get_can_be_deleted() {
	return can_be_deleted;
}

void Piece::set_can_be_deleted(bool b) {
	can_be_deleted = b;
}
...
void Piece::update_Piece_position() {
	Starting_Piece_pos = getPosition();
}
...
void Piece::legal_move_avoider(std::vector<BoardTile*>& board, std::vector<Piece*>& _PawnsVec) {
	if (last_move_was_Black == true) {
		auto itr = std::find_if(_PawnsVec.begin(), _PawnsVec.end(), [](Piece* _piece) {
			return _piece->get_Piece_type() == K && _piece->get_Piece_color() == Black; });

		auto Black_king_pos = std::find_if(board.begin(), board.end(), [itr](BoardTile* Tile) {
			return Tile->get_Tile_position() == (*itr)->getPosition(); });
		...
		for (auto& el : _PawnsVec) {
			if (el->get_delectable() == false) {
				el->mark_Tiles(board, _PawnsVec);
			}
		}
		if ((*Black_king_pos)->get_Tile_marked_for_White() == true) {
			setPosition(Starting_Piece_pos);
			last_move_was_Black = false;
		}
	}
	else if (last_move_was_Black == false) {

		auto itr = std::find_if(_PawnsVec.begin(), _PawnsVec.end(), [](Piece* _piece) {
			return _piece->get_Piece_type() == K && _piece->get_Piece_color() == White; });

		auto White_king_pos = std::find_if(board.begin(), board.end(), [itr](BoardTile* Tile) {
			return Tile->get_Tile_position() == (*itr)->getPosition(); });
		...
		if ((*White_king_pos)->get_Tile_marked_for_Black() == true) {
			setPosition(Starting_Piece_pos);
			last_move_was_Black = true;
		}
	}
...
bool Piece::_collider_for_BlackRook(std::vector<Piece*>& _PawnsVec, sf::Vector2f selected_Tile_pos) {
	auto itr = std::find_if(_PawnsVec.begin(), _PawnsVec.end(), [selected_Tile_pos](Piece* _piece) {
		return selected_Tile_pos == _piece->getPosition();
		});
	if ((
...
bool Piece::_collider_for_WhiteRook(std::vector<Piece*>& _PawnsVec, sf::Vector2f selected_Tile_pos) {
	auto itr = std::find_if(_PawnsVec.begin(), _PawnsVec.end(), [selected_Tile_pos](Piece* _piece) {
		return selected_Tile_pos == _piece->getPosition();
		});
	if ((
...
bool Piece::_collider_for_BlackBishop(std::vector<Piece*>& _PawnsVec, sf::Vector2f selected_Tile_pos) {
	auto itr = std::find_if(_PawnsVec.begin(), _PawnsVec.end(), [selected_Tile_pos](Piece* _piece) {
		return selected_Tile_pos == _piece->getPosition();
		});
	if ((
...
bool Piece::_collider_for_WhiteBishop(std::vector<Piece*>& _PawnsVec, sf::Vector2f selected_Tile_pos) {
	auto itr = std::find_if(_PawnsVec.begin(), _PawnsVec.end(), [selected_Tile_pos](Piece* _piece) {
		return selected_Tile_pos == _piece->getPosition();
		});
	if ((
...
Piece* Piece::_collider_for_tie_BlackRook(std::vector<Piece*>& _PawnsVec, sf::Vector2f selected_Tile_pos) {
	auto itr = std::find_if(_PawnsVec.begin(), _PawnsVec.end(), [selected_Tile_pos](Piece* _piece) {
		return selected_Tile_pos == _piece->getPosition();
		});
	if ((
		...
		if (itr != _PawnsVec.end() && (*itr)->get_Piece_type() == K && (get_Piece_type() == R || get_Piece_type() == R1 || get_Piece_type() == Q)) {
			sf::Vector2f chosen_pos = selected_Tile_pos;
			sf::Vector2f starting_pos = get_Starting_Piece_pos();
			int counter = 0;
			Piece* pie = NULL;
			...
			if (chosen_pos.x - starting_pos.x > 0) {
				...
				if (counter == 1 && (*itr)->get_Piece_color() == White) {
					return pie;
				}
				else {
					return NULL;
...
Piece* Piece::_collider_for_tie_WhiteRook(std::vector<Piece*>& _PawnsVec, sf::Vector2f selected_Tile_pos) {
	auto itr = std::find_if(_PawnsVec.begin(), _PawnsVec.end(), [selected_Tile_pos](Piece* _piece) {
		return selected_Tile_pos == _piece->getPosition();
		});
	if ((
		...
		if (itr != _PawnsVec.end() && (*itr)->get_Piece_type() == K && (get_Piece_type() == R || get_Piece_type() == R1 || get_Piece_type() == Q)) {
			...
			if (chosen_pos.x - starting_pos.x > 0) {
				...
				if (counter == 1 && (*itr)->get_Piece_color() == Black) {
					return pie;
				}
				else {
					return NULL;
...
Piece* Piece::_collider_for_tie_BlackBishop(std::vector<Piece*>& _PawnsVec, sf::Vector2f selected_Tile_pos) {
	auto itr = std::find_if(_PawnsVec.begin(), _PawnsVec.end(), [selected_Tile_pos](Piece* _piece) {
		return selected_Tile_pos == _piece->getPosition();
		});
	if ((
		...
		if (itr != _PawnsVec.end() && (*itr)->get_Piece_type() == K && (get_Piece_type() == B || get_Piece_type() == Q)) {
			sf::Vector2f chosen_pos = selected_Tile_pos;
			sf::Vector2f starting_pos = get_Starting_Piece_pos();
			int counter = 0;
			Piece* pie = NULL;
...
Piece* Piece::_collider_for_tie_WhiteBishop(std::vector<Piece*>& _PawnsVec, sf::Vector2f selected_Tile_pos) {
	auto itr = std::find_if(_PawnsVec.begin(), _PawnsVec.end(), [selected_Tile_pos](Piece* _piece) {
		return selected_Tile_pos == _piece->getPosition();
		});
	if ((
